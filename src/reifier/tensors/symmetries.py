...
def lognormal(like: t.Tensor, mean=0., std=1., signed=True):
    s = t.empty(like.shape[0], dtype=like.dtype, device=like.device).log_normal_(mean, std)
    if signed:
        s.mul_(random_signs(s))
    return s

def random_orthogonal(n, device='cuda', dtype=t.float32) -> t.Tensor:
    """Generates a random orthogonal matrix via Stewart's Algorithm."""
    X = t.randn(n, n, dtype=dtype, device=device).tril_()
    d = X.diagonal()
    norms = X.norm(dim=0).clamp_(min=t.finfo(dtype).eps)
    v = d + t.copysign(norms, d)
    tau = 1 + d.abs() / norms
    X.div_(v)
    return t.linalg.householder_product(X, tau).mul_(random_signs(norms))

def set_wn_to_ones(g) -> None:
...
def scale_norm(g, mean=0., std=1.) -> None:
...
def scale_hidden(g, mean=0., std=1., wg_min=1.) -> None:
